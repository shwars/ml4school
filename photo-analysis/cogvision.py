...
class CognitiveServicesFace():

    def __init__(self,key,region):
        self.key = key
        self.region = region
        self.url = 'https://{}.api.cognitive.microsoft.com/face/v1.0/detect'.format(region)

    def _postproc(self,result):
        return result


    def detect(self,img, attributes="age,gender"):

        # Computer Vision parameters
        params = {'returnFaceAttributes': attributes, 'returnFaceId' : False, 'recognitionModel' : 'recognition_02' }
    ...
    def detect_file(self,filename,attributes="age,gender"):
        with open(filename, 'rb') as f:
            data = f.read()
        return self.detect(data,attributes=attributes)

    def detect_url(self,image_url, attributes="age,gender"):
